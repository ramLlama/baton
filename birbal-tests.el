...
;; birbal agent registry, and birbal-monet.
;;
;; Run via Makefile:
;;   make test
...
(defmacro birbal-test-with-clean-state (&rest body)
  ...
  `(let ((birbal--sessions (make-hash-table :test 'equal))
         ...
         (birbal-agents (make-hash-table :test 'eq))
...
(ert-deftest birbal-test-session-create-returns-struct ()
  ...
  (birbal-test-with-clean-state
    (let ((s (birbal-session-create :agent 'claude-code
                                    :command "claude"
                                    :directory "/tmp")))
...
(ert-deftest birbal-test-session-create-duplicate-name-errors ()
  ...
  (birbal-test-with-clean-state
    (birbal-session-create :agent 'claude-code :command "claude"
                           :directory "/tmp" :name "my-agent")
    (should-error (birbal-session-create :agent 'claude-code :command "claude"
                                         :directory "/tmp" :name "my-agent")
                  :type 'error)))
...
(ert-deftest birbal-test-session-create-auto-name ()
  ...
  (birbal-test-with-clean-state
    (let ((s1 (birbal-session-create :agent 'claude-code
                                     :command "claude"
                                     :directory "/tmp"))
          (s2 (birbal-session-create :agent 'claude-code
                                     :command "claude"
                                     :directory "/tmp")))
...
(ert-deftest birbal-test-session-list-all ()
  ...
  (birbal-test-with-clean-state
    (birbal-session-create :agent 'claude-code :command "claude" :directory "/tmp")
    (birbal-session-create :agent 'aider       :command "aider"  :directory "/tmp")
...
(ert-deftest birbal-test-session-list-filter-by-status ()
  ...
  (birbal-test-with-clean-state
    (let ((s1 (birbal-session-create :agent 'claude-code :command "claude" :directory "/tmp"))
          (s2 (birbal-session-create :agent 'aider       :command "aider"  :directory "/tmp")))
...
(ert-deftest birbal-test-session-set-status ()
  ...
  (birbal-test-with-clean-state
    (let ((s (birbal-session-create :agent 'claude-code :command "claude" :directory "/tmp"))
...
(ert-deftest birbal-test-define-agent-stores ()
  "birbal-define-agent stores the definition at the symbol key."
  (birbal-test-with-clean-state
    (birbal-define-agent :name 'my-agent
                              :command "my-cmd"
                              :waiting-patterns '((">" . "prompt")))
    (should (gethash 'my-agent birbal-agents))))

(ert-deftest birbal-test-define-agent-retrieval ()
  ...
  (birbal-test-with-clean-state
    ...
    (should (equal "my-cmd"
                   (plist-get (gethash 'my-agent birbal-agents) :command)))))

(ert-deftest birbal-test-agent-waiting-patterns ()
  "Waiting patterns are stored as an alist of (pattern . reason)."
  (birbal-test-with-clean-state
    (birbal-define-agent :name 'my-agent
                              :command "cmd"
                              :waiting-patterns '(("^> " . "input") ("Allow" . "permission")))
    (let ((patterns (plist-get (gethash 'my-agent birbal-agents) :waiting-patterns)))
...
(ert-deftest birbal-test-notify-modeline-with-waiting ()
  ...
  (birbal-test-with-clean-state
    (let ((s1 (birbal-session-create :agent 'claude-code :command "claude" :directory "/tmp"))
          (s2 (birbal-session-create :agent 'claude-code :command "claude" :directory "/tmp"))
          (s3 (birbal-session-create :agent 'claude-code :command "claude" :directory "/tmp")))
      (birbal-session-set-status s1 'waiting "prompt")
      (let ((str (birbal-notify--modeline-string)))
...
(ert-deftest birbal-test-status-buffer-sorted ()
  ...
  (birbal-test-with-clean-state
    (let ((s1 (birbal-session-create :agent 'claude-code :command "claude" :directory "/a"))
          (s2 (birbal-session-create :agent 'aider       :command "aider"  :directory "/b")))
...
(ert-deftest birbal-test-monet-find-session-prefers-claude-code ()
  ...
  (birbal-test-with-clean-state
    (let ((s-aider  (birbal-session-create :agent 'aider
                                           :command "aider"
                                           :directory "/proj"))
          (s-claude (birbal-session-create :agent 'claude-code
                                           :command "claude"
                                           :directory "/proj")))
...
(ert-deftest birbal-test-monet-setup-enables-birbal-set ()
  ...
  (birbal-test-with-clean-state
    (birbal-define-agent :name 'claude-code :command "claude" :waiting-patterns nil)
...
(ert-deftest birbal-test-env-functions-nil-by-default ()
  "birbal-define-agent without :env-functions stores nil."
  (birbal-test-with-clean-state
    (birbal-define-agent :name 'test-agent :command "cmd" :waiting-patterns nil)
    (should (null (plist-get (gethash 'test-agent birbal-agents) :env-functions)))))
...
(ert-deftest birbal-test-env-functions-add-appends ()
  ...
  (birbal-test-with-clean-state
    ...
    (let ((fn1 (lambda (_k _d) '("A=1")))
      ...
      (let ((fns (plist-get (gethash 'test-agent birbal-agents) :env-functions)))
        (should (= 2 (length fns)))
...
(ert-deftest birbal-test-env-functions-nil-produces-no-extra-env ()
  ...
  (birbal-test-with-clean-state
    (birbal-define-agent :name 'test-agent :command "cmd" :waiting-patterns nil)
    (let* ((def (gethash 'test-agent birbal-agents))
