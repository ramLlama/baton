...
28. **The sandbox accepts only ONE concurrent connection (microsandbox SDK).** Verified empirically (2026-06-10): sequential clean exec/attach connections work, but any concurrent second connection fails with `failed to connect … already running`, and a *killed* client leaks the connection slot, wedging the sandbox until remove/start. This is why the port forwarders are **in-guest socats backgrounded by the attach command's wrapper shell** (one connection total) and must never be separate `sodagun sandbox exec` clients. If in-guest orchestration ever outgrows the `sh -c '… & exec …'` wrapper, build on process-compose rather than extra connections.

29. **Monet-in-sandbox needs `~/.claude` bind-mounted into the guest (live).** Claude discovers the MCP server through the lockfile `~/.claude/ide/<Pm>.lock` (auth token + workspaceFolders) written on the *host*, and the hook command is `$HOME/.claude/hooks/monet-claude-hook.py`. The sandbox's `sodagun.toml` must bind-mount the host `~/.claude` into the guest home (verified live in both directions), or Claude-in-guest finds neither. This is the user's sodagun.toml responsibility, **not** baton's. Note the lockfile pid is **per-session** (monet LOCKFILE-PID, fed by `baton-executor-guest-pid`): sandbox sessions advertise pid 1 (a host Emacs pid would not exist in the guest pid namespace and Claude *deletes* lockfiles with dead pids — through the live mount, erasing them host-side too), while host sessions advertise `emacs-pid` (on macOS, `kill(1, 0)` from userspace is EPERM, which Claude treats as dead — pid-1 lockfiles get reaped by any interactive host claude startup or `/ide` scan; verified empirically 2026-06-12, and **no pid validates in both namespaces**). Sandbox lockfiles therefore remain exposed to host claude reaping; tabled fix is a guest-private `~/.claude/ide` (sodagun `claude_access`).

30. **Guest/host path mismatches are handled by path mappings, not rewrites.** The guest sees the worktree at `baton-sodagun-guest-workdir` (default `/workspace`; must match sodagun.toml `working_dir`). The sodagun executor binds `baton-executor-guest-path-mappings` to `((WORKTREE . "/workspace"))` around env evaluation; baton-monet forwards it to monet, which lists the guest folder in the lockfile and translates protocol paths in both directions. Verified working including diff review (2026-06-11).

31. **The guest image must ship `socat`, `python3`, and (for in-guest git) `git`.** The per-port forwarders are in-guest `socat`s backgrounded by the attach command's wrapper shell; monet's Claude hook script is a stdlib-only `python3` script; in-guest git works via sodagun's `git_access` config. Provisioning is the user's image responsibility, not baton's. (A missing socat prints a warning in the session terminal; the agent still runs, without monet bridges.)
...
34. **Spawn env must be installed on the DEFAULT value of `process-environment`, never `setq`.** `baton-process-spawn` `cl-letf`-binds `(default-value 'process-environment)` to extra-env + the calling buffer's env. With envrc/direnv active, the calling buffer's `process-environment` is *buffer-local*; a plain `setq` (the old code) writes that buffer-local binding, so the terminal buffer — which reads the default value after the buffer switch — never sees extra-env (monet's ports silently vanish), and the calling buffer's envrc env accumulates stale per-session vars. The failure is invocation-buffer-dependent: spawning from a non-envrc buffer works, from an envrc-managed project buffer breaks.
