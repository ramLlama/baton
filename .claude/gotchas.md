...
31. **The guest image must ship `socat` and `python3`.** The per-port forwarders are in-guest `socat`s backgrounded by the attach command's wrapper shell, and monet's Claude hook script is a stdlib-only `python3` script that runs in-guest. Both must be present in the user's sandbox image — provisioning them is the user's image responsibility, not baton's. (A missing socat prints a warning in the session terminal; the agent still runs, without monet bridges.)
