...
29. **Monet-in-sandbox needs `~/.claude` bind-mounted into the guest (live).** Claude discovers the MCP server through the lockfile `~/.claude/ide/<Pm>.lock` (auth token + workspaceFolders) written on the *host*, and the hook command is `$HOME/.claude/hooks/monet-claude-hook.py`. The sandbox's `sodagun.toml` must bind-mount the host `~/.claude` into the guest home (verified live in both directions), or Claude-in-guest finds neither. This is the user's sodagun.toml responsibility, **not** baton's. Note monet writes the lockfile with **pid 1** — a host Emacs pid would not exist in the guest pid namespace and Claude *deletes* lockfiles with dead pids (through the live mount, erasing them host-side too).
